...
class Maze:
    ...
    def __init__(self, maze_cells=None, maze_size=(10,10), has_loops=True, num_portals=0):
        ...
        # Set matrices para los colores de las paredes
        self.color_lines_hor = np.ones((self.maze_size[0]+1, self.maze_size[1]), dtype=int)*12
        self.color_lines_ver = np.ones((self.maze_size[0], self.maze_size[1]+1), dtype=int)*12
        # Set borders en negro
        self.color_lines_hor[0, :] = 255 * np.ones(self.maze_size[1], dtype=int)
        self.color_lines_hor[self.maze_size[0], :] = 255 * np.ones(self.maze_size[1], dtype=int)
        self.color_lines_ver[:, 0] = 255 * np.ones(self.maze_size[0], dtype=int)
        self.color_lines_ver[:, self.maze_size[1]] = 255 * np.ones(self.maze_size[0], dtype=int)
    ...
    def color_walls(self):
        # breaking the walls
        for x in range(len(self.maze_cells)):
            for y in range(len(self.maze_cells[x])):
                # check the which walls are open in each cell
                walls_status = self.get_walls_status(self.maze_cells[x, y])
                dirs = ""
                for dir, open in walls_status.items():
                    if open:
                        if dir == 'W':
                            self.color_lines_ver[y, x] = 0
                        elif dir == 'E':
                            self.color_lines_ver[y, x+1] = 0
                        elif dir == 'N':
                            self.color_lines_hor[y, x] = 0
                        elif dir == 'S':
                            self.color_lines_hor[y+1, x] = 0
                        else:
